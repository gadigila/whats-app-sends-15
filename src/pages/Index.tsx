...
async function updateProgress(supabase: any, progress: SyncProgress) {
  ...
  try {
    ...
    if (healthResponse.ok) {
      ...
      if (healthData.status === 'SYNC_ERROR') {
        ...
        // Recovery strategy: wait longer and update status
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const passConfig = [
      ...
      { pass: 2, delay: 10000, batchSize: 100, description: "גילוי מפורט" },
      { pass: 3, delay: 15000, batchSize: 150, description: "סריקה מקיפה אחרונה" }
    ];
    ...
    try {
      ...
      const { userId } = await req.json().catch(() => ({ userId: 'unknown' }))
